...
Deno.serve(async (req) => {
  try {
    ...
    const application = await base44.asServiceRole.entities.GuestProductionApplication.create({
      ...
      source_page: payload.source_page || 'become_performer', source_country: payload.source_country || null,
      landing_page: payload.landing_page || payload.source_page || null, recruitment_page: payload.recruitment_page || payload.source_page || null,
      referrer: payload.referrer || null, source: payload.source || payload.utm_source || null, medium: payload.medium || payload.utm_medium || null, campaign: payload.campaign || payload.utm_campaign || null, country: payload.country || payload.source_country || null,
      first_touch_source: payload.first_touch_source || null, first_touch_landing_page: payload.first_touch_landing_page || null,
      utm_source: payload.utm_source || null, utm_market: payload.utm_market || null, utm_campaign: payload.utm_campaign || null, utm_medium: payload.utm_medium || null, utm_content: payload.utm_content || null, utm_term: payload.utm_term || null,
      referral_code: payload.referral_code || null, recruitment_campaign_id: payload.recruitment_campaign_id || null,
