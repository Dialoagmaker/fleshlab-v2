...
function MissionSection({ mission, onMissionAction }) {
  ...
  const canSkip = mission && mission.status !== "completed";
  return <div className="flos-detail-grid"><article><h3><Target /> Mission status</h3><p>{mission?.title || "No mission available yet. Creator OS will generate one from the next briefing."}</p><small>{mission?.status || "pending"}</small><div className="flos-detail-actions"><button disabled={!canStart} onClick={() => onMissionAction("start")}>Begin</button><button disabled={!canPause} onClick={() => onMissionAction("pause")}>Pause</button><button disabled={!canResume} onClick={() => onMissionAction("resume")}>Resume</button><button disabled={!canComplete} onClick={() => onMissionAction("complete")}>Complete</button><button disabled={!canSkip} onClick={() => onMissionAction("skip")}>Skip</button></div></article><article><h3><Video /> Mission steps</h3>{list(mission?.steps)}</article><article><h3><Clock /> Equipment and duration</h3>{list(mission?.equipment)}<small>{mission?.minutes ? `${mission.minutes} minutes estimated` : "No duration yet"}</small></article><article><h3><BookOpen /> Content goal</h3><p>{mission?.contentGoal || "No content goal available yet."}</p><h3>AI guidance</h3><p>{mission?.aiGuidance || "AI guidance appears after the mission is generated."}</p></article></div>;
}
