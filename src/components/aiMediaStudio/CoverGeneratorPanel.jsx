...
export default function CoverGeneratorPanel({ item }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!frame ? <Card><CardContent className="p-6 text-sm font-semibold text-destructive">No available hero frame is selected. Choose a ranked frame from the Creative Director gallery.</CardContent></Card> : !generationStarted ? <Card><CardContent className="p-6 text-sm text-muted-foreground">Select a Hero Frame to lock the photograph and generate KRAKEN cover candidates.</CardContent></Card> : coverMode === "v3" ? <CoverV3Mode frame={frame} compareFrames={compareFrames} lockedHero={lockedHeroFrame} metadata={rendererMetadata} settings={settings} itemFileName={item?.fileName} onUseFallback={() => setCoverMode("local")} /> : coverMode === "local" ? <CoverVariantCompare frame={frame} candidateFrames={compareFrames.length ? compareFrames : heroCandidates} metadata={rendererMetadata} settings={settings} itemFileName={item?.fileName} /> : <OpenRouterCoverMode frame={frame} metadata={rendererMetadata} settings={settings} />}
    </div>
  );
}