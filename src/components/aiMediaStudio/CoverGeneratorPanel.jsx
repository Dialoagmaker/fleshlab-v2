...
export default function CoverGeneratorPanel({ item }) {
  ...
  const heroCandidates = useMemo(() => rankEmotionalCommercialFrames(item?.frames || [], 10, 45), [item?.frames]);
  ...
  const generateAutomaticCover = () => {
    ...
    setMetadata(current => ({
      ...current,
      videoTitle: current.videoTitle || item?.fileName?.replace(/\.[^/.]+$/, "") || "FLESHLAB ORIGINAL",
      optionalSubtitle: current.optionalSubtitle || consensus?.commercialPromise || "Every room has a secret.",
      contentType: current.contentType || "Hotel Sessions",
      campaignName: current.campaignName || consensus?.campaignName || "Hotel Sessions",
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-muted-foreground">
          <p>Analyze a video first. The system searches 1000+ frames for emotional commercial potential before the Creative Director starts.</p>
        </CardContent>
      </Card>
    );
  }
