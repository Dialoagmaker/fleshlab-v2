...
export default function CoverPresetControls({ settings, onChange }) {
  ...
  const patchPreset = value => onChange({ presetId: value, variant: value === "v2-title" ? "title" : value === "v2-performer" ? "performer" : value === "v2-balanced" ? "balanced" : "auto" });
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <div className="space-y-1"><Label className="text-xs">Template preset</Label><Select value={settings.presetId} onValueChange={patchPreset}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{COVER_PRESETS.map(preset => <SelectItem key={preset.id} value={preset.id}>{preset.label}</SelectItem>)}</SelectContent></Select></div>
      <div className="space-y-1"><Label className="text-xs">Variant</Label><Select value={settings.variant || "auto"} onValueChange={value => patch({ variant: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="auto">Auto ranked winner</SelectItem><SelectItem value="title">Title Dominant</SelectItem><SelectItem value="performer">Performer Dominant</SelectItem><SelectItem value="balanced">Balanced</SelectItem></SelectContent></Select></div>
