...
function FooterLink({ href, children }) {
  return <a href={href} className="group/link relative inline-flex w-fit py-1.5 text-sm text-white/54 transition duration-300 hover:translate-x-1 hover:text-white"><span>{children}</span><span className="absolute bottom-1 left-0 h-px w-0 bg-[#E51D2A] transition-all duration-300 group-hover/link:w-full" /></a>;
}
...
export default function FinalBrandFooter() {
  ...
  return (
    <footer className="relative overflow-hidden bg-[#040506] text-white">
      ...
      <section className="relative mx-auto max-w-[1360px] px-5 py-14 lg:px-7 lg:py-20">
        <div className="max-w-5xl">
          <p className="mb-6 text-[10px] font-black uppercase tracking-[0.36em] text-[#E51D2A]">The final scene</p>
          ...
          <p className="mt-6 max-w-2xl text-base font-semibold leading-7 text-white/68 md:text-lg md:leading-8">Meet the creators behind authentic amateur productions — real people building real stories, one release at a time.</p>
          <a href="/performers" className="group mt-10 inline-flex items-center gap-4 rounded-full bg-[#E51D2A] px-8 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-white shadow-[0_24px_80px_rgba(229,29,42,0.28)] transition duration-300 hover:-translate-y-1 hover:bg-[#ff3340] hover:shadow-[0_30px_100px_rgba(229,29,42,0.42)]">
      ...
      <section className="relative border-y border-white/10">
        <div className="mx-auto grid max-w-[1360px] gap-10 px-5 py-10 md:py-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-start lg:px-7">
          <div className="max-w-xl">
            ...
            <p className="mt-6 text-[34px] font-black uppercase leading-none tracking-[-0.07em] md:text-[48px]">AMATEUR WINS.</p>
            <p className="mt-4 max-w-xl text-sm leading-6 text-white/58 md:text-base">Built around real creators, independent productions and moments that feel less polished, more human and impossible to fake.</p>
          ...
          <nav className="grid grid-cols-2 gap-x-10 gap-y-10 pt-1 md:grid-cols-4" aria-label="Footer navigation">
            {navGroups.map((group) => <div key={group.title}><h3 className="mb-5 text-[10px] font-black uppercase tracking-[0.28em] text-white">{group.title}</h3><div className="flex flex-col">{group.links.map((link) => <FooterLink key={link.label} href={link.href}>{link.label}</FooterLink>)}</div></div>)}
          </nav>
        </div>
      </section>

      <section className="relative mx-auto flex max-w-[1360px] flex-col gap-5 px-5 py-5 text-[9px] font-black uppercase tracking-[0.16em] text-white/46 lg:flex-row lg:items-center lg:justify-between lg:px-7">
