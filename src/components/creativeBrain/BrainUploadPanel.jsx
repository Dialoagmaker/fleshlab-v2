  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_220px_220px_auto] lg:items-end">
        <label><span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Upload Hero Frame</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { classifyHeroFrameFile(event.target.files?.[0]); onFileChange(event); }} className="mt-2 block w-full rounded-lg border border-input bg-background p-3 text-sm text-foreground" /></label>
