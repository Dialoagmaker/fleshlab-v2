export default function CampaignComposerComparison({ improvedAssets = [] }) {
  ...
  return (
    <div className="mt-5 rounded-xl border border-primary/30 bg-background/50 p-4">
      ...
      <p className="mt-3 text-[11px] font-bold uppercase tracking-widest text-muted-foreground">{[asset.compositionPlan?.creativeConcept?.campaignIdentity || asset.campaignMetadata?.campaignTitle, asset.campaignMetadata?.performerName, asset.compositionPlan?.layoutFamily, "Layered depth"].filter(Boolean).join(" · ")}</p>
    </div>
  );
}