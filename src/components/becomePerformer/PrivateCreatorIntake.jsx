...
export default function PrivateCreatorIntake({ onVerifyClick }) {
  ...
  const submit = useMutation({
    ...
    onMutate: () => trackEvent("recruitment_private_intake_submit", { creator_path: selectedPath, interests_count: form.interests.length }),
  });
    ...
    <div id="private-creator-intake" className="rounded-[2rem] border border-primary/25 bg-card p-6 shadow-2xl shadow-primary/10 md:p-8">
      ...
      {submit.isSuccess ? <CreatorPathResult path={selectedPath} onVerifyClick={onVerifyClick} /> : <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/8 p-4"><label className="flex items-start gap-3"><Checkbox checked={form.age} onCheckedChange={(age) => setForm({ ...form, age: !!age })} /><span className="text-sm leading-6 text-muted-foreground">I confirm I am 18 years or older and understand FLESHLAB is adult creator work, not escorting or dating. *</span></label></div>
        </div>

        <div className="space-y-5">
          <div><Label className="mb-2 block text-xs uppercase tracking-widest text-muted-foreground">Which sounds most like you?</Label><div className="grid gap-2 sm:grid-cols-2">{PATHS.map(([value, label, description]) => <IntakeOptionButton key={value} label={label} description={description} selected={form.path === value} onClick={() => { setForm({ ...form, path: value }); trackEvent("recruitment_creator_path_selected", { creator_path: value }); }} />)}</div></div>
