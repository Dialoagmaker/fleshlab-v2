...
export default function CatalogueImport() {
  ...
  return <>
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {report && <section className={`rounded-2xl border p-5 md:p-6 ${report.dry_run ? 'border-amber-500/35 bg-amber-500/5' : 'border-emerald-500/35 bg-emerald-500/5'}`}><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-black uppercase tracking-[.18em] text-muted-foreground">{report.dry_run ? 'Dry run passed' : 'Import complete'}</p><h2 className="mt-1 text-xl font-black text-foreground">{report.dry_run ? 'Review this catalogue snapshot' : 'Catalogue records were imported'}</h2><p className="mt-2 text-sm text-muted-foreground">Source checksum: <code className="break-all text-xs">{report.source_sha256}</code></p></div>{report.dry_run && <button type="button" className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700 disabled:opacity-50" disabled={importing} onClick={() => void execute()}>{importing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}Confirm and import</button>}</div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Metric label="Brands" value={report.brands} /><Metric label="Performers" value={report.performers} /><Metric label="Videos" value={report.videos} /><Metric label="Valid credits" value={report.credits} /><Metric label="Skipped credits" value={report.skipped_credits} /></div>{report.skipped_credits > 0 && <div className="mt-5 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm leading-6 text-amber-900 dark:text-amber-100"><b>{report.skipped_credits} unresolved credit{report.skipped_credits === 1 ? '' : 's'} will not be imported.</b> The complete records remain importable. Re-export the missing parent records later, then replay this same snapshot idempotently. {report.unresolved_credits?.slice(0, 10).map((issue, index) => <div key={`${issue.credit_id || index}-${issue.video_id}`} className="mt-2 font-mono text-xs">{issue.reference}: {issue.missing.map(item => `${item.entity} ${item.id}`).join(', ')}</div>)}</div>}{report.dry_run && <p className="mt-5 text-sm leading-6 text-muted-foreground">No data has been changed. Confirm the counts and relationships, then select <b>Confirm and import</b>. Existing matching legacy IDs are updated idempotently.</p>}</section>}
    </div>
  </>;
}
