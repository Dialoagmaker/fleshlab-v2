...
export class V3ContractService {
  ...
  async template(id) { const result = await this.db.query('SELECT * FROM v3_contract_templates WHERE id::text=$1 OR template_key=$1', [id]); if (!result.rowCount) throw new HttpError(404,'TEMPLATE_NOT_FOUND','Contract template was not found.'); const versions = await this.db.query('SELECT id,template_id,version,body,sections,variables,body_hash,created_at FROM v3_contract_template_versions WHERE template_id=$1 ORDER BY created_at DESC',[result.rows[0].id]); return { template: result.rows[0], versions: versions.rows }; }
  async createVersion(templateId, input, actor) {
    const template = await this.db.query('SELECT * FROM v3_contract_templates WHERE id::text=$1 OR template_key=$1',[templateId]); if (!template.rowCount) throw new HttpError(404,'TEMPLATE_NOT_FOUND','Contract template was not found.');
  ...
  async instance(id,user,{admin=false}={}) { const query=admin?`SELECT i.*,t.title,t.contract_type,v.version FROM v3_contract_instances i LEFT JOIN v3_contract_template_versions v ON v.id=i.template_version_id LEFT JOIN v3_contract_templates t ON t.id=v.template_id WHERE i.id=$1`:`SELECT i.*,t.title,t.contract_type,v.version FROM v3_contract_instances i JOIN v3_creator_records c ON c.id=i.creator_id LEFT JOIN v3_contract_template_versions v ON v.id=i.template_version_id LEFT JOIN v3_contract_templates t ON t.id=v.template_id WHERE i.id=$1 AND c.user_id=$2`; const result=await this.db.query(query,admin?[id]:[id,user.id]); if(!result.rowCount)throw new HttpError(404,'CONTRACT_NOT_FOUND','Contract was not found.'); const row=result.rows[0]; if(!row.viewed_at){await this.db.query('UPDATE v3_contract_instances SET viewed_at=now(),status=CASE WHEN status=\'assigned\' THEN \'viewed\' ELSE status END WHERE id=$1',[id]); await this.event('contract.viewed',user,id,row.template_id || null,{});} return { ...safe(row), rendered_snapshot:row.rendered_snapshot };
  }
