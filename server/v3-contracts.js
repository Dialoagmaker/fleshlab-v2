...
export const contractInstanceStates = new Set(['assigned','viewed','signed','cancelled','superseded','legacy_imported']);
...
export class V3ContractService {
  ...
  async assign(input, actor) {
    const version = await this.db.query(`SELECT v.*,t.template_key,t.title,t.status FROM v3_contract_template_versions v JOIN v3_contract_templates t ON t.id=v.template_id WHERE v.id=$1`,[input.template_version_id]); if (!version.rowCount) throw new HttpError(404,'TEMPLATE_VERSION_NOT_FOUND','Template version was not found.');
    if (version.rows[0].status !== 'approved') throw new HttpError(409,'LEGAL_REVIEW_REQUIRED','Only an approved template can be assigned.');
    const creator = await this.db.query(`SELECT c.id,c.full_name,c.private_profile,p.display_name FROM v3_creator_records c LEFT JOIN v3_creator_performer_links l ON l.creator_id=c.id LEFT JOIN catalog_performers p ON p.legacy_id=l.performer_legacy_id WHERE c.id=$1`,[input.creator_id]); if (!creator.rowCount) throw new HttpError(404,'CREATOR_NOT_FOUND','Creator was not found.');
    const number = String(input.contract_number || `FL-${new Date().getUTCFullYear()}-${crypto.randomBytes(5).toString('hex').toUpperCase()}`); const values={ company_legal_name:'Dialogmakers International Ltd.', company_number:'83273694', creator_legal_name:input.creator_legal_name || creator.rows[0].full_name, performer_name:input.performer_name || creator.rows[0].display_name || '', contract_number:number, effective_date:input.effective_date || new Date().toISOString().slice(0,10), template_version:version.rows[0].version, contract_status:'assigned' }; const snapshot=replaceVariables(version.rows[0].body,values); const instance=await this.db.query(`INSERT INTO v3_contract_instances(template_version_id,creator_id,contract_number,status,rendered_snapshot,snapshot_hash,creator_legal_name,performer_name,created_by) VALUES($1,$2,$3,'assigned',$4,$5,$6,$7,$8) RETURNING *`,[version.rows[0].id,creator.rows[0].id,number,snapshot,hash(snapshot),values.creator_legal_name,values.performer_name,actor.id]); await this.event('contract.assigned',actor,instance.rows[0].id,version.rows[0].template_id,{ contract_number:number }); return safe(instance.rows[0]);
  }
