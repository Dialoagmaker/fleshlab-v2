...
const performerPlace = performer => String(performer.public_location || performer.nationality || '').trim() || 'FLESHLAB FILE';
const performerLabels = performer => Array.isArray(performer.labels) ? performer.labels.filter(label => label?.name) : [];
...
function FleshNav() {
  const links = <><a href="/v3/videos">Archive</a><a href="/v3/performers">Performers</a><a href="/v3/collections">Series</a><a href="/v3/brands">Labels</a></>;
...
function FleshFooter() {
  return <footer className="flesh-footer">
    ...
    <div><span>ARCHIVE</span><a href="/v3/videos">All tapes</a><a href="/v3/performers">Performers</a><a href="/v3/collections">Series</a><a href="/v3/brands">Labels</a><a href="https://earn.fleshlab.online">Earn</a></div>
...
function PerformerSheet({ performer, index = 0 }) { const labels = performerLabels(performer); return <a className="performer-sheet" href={performerHref(performer)}><Image record={performer} alt={`Portrait of ${performer.display_name}`}/><div className="sheet-mark">#{String(index + 1).padStart(3, '0')}</div><div className="sheet-caption"><div><strong>{esc(performer.display_name)}</strong><span>{esc(performerPlace(performer))}</span></div><b aria-hidden="true">+</b></div>{labels.length ? <div className="sheet-labels"><span>{labels.length > 1 ? 'LABELS' : 'LABEL'} /</span> {labels.map((label, labelIndex) => <React.Fragment key={label.id || label.slug || labelIndex}>{labelIndex > 0 ? ' · ' : ''}{esc(label.name)}</React.Fragment>)}</div> : null}</a>; }
...
function Listing({ kind }) {
  const initialLabel = new URLSearchParams(window.location.search).get('label') || ''; const [data, setData] = useState(null); const [q, setQ] = useState(''); const [labelFilter] = useState(initialLabel); const [error, setError] = useState('');
  const load = () => get(`/${kind}?q=${encodeURIComponent(q)}&limit=40${kind === 'performers' && labelFilter ? `&label=${encodeURIComponent(labelFilter)}` : ''}`).then(setData).catch(e => setError(e.message)); useEffect(() => { load(); }, [kind, labelFilter]);
  const records = data?.records || []; const title = kind === 'videos' ? 'THE ARCHIVE' : kind === 'performers' ? 'FLESHLAB PERFORMERS' : 'THE LABELS'; const description = kind === 'videos' ? 'Unfiltered releases from FLESHLAB.' : kind === 'performers' ? 'REAL GUYS. DIFFERENT PLACES. SAME ENERGY.' : `Public ${kind} files from the FLESHLAB world.`;
  const labelOptions = [['', 'ALL'], ['fleshlabasia', 'FLESHLAB ASIA'], ['fleshlab-twinks-global', 'FLESHLAB TWINKS GLOBAL'], ['fleshlab-bareback-twinks', 'FLESHLAB BAREBACK TWINKS']];
  return <Shell seo={{ title: `${title} / FLESHLAB`, description }}><main className="public-main public-listing"><section className="archive-title"><span className="archive-kicker">FLESHLAB / PUBLIC INDEX</span><h1>{title}</h1><p>{description}</p></section>{kind === 'videos' && <div id="search-videos" className="archive-controls"><label className="archive-search"><span>SEARCH THE FILES</span><input value={q} onChange={event => setQ(event.target.value)} onKeyDown={event => event.key === 'Enter' && load()} placeholder="title, performer, label"/><button onClick={load}>FIND ↗</button></label><div className="archive-filters"><a href="/v3/videos">ALL</a><a href="/v3/videos?q=latest">LATEST</a><a href="/v3/performers">PERFORMER</a><a href="/v3/collections">SERIES</a><a href="/v3/brands">LABEL</a></div></div>}{kind === 'performers' && <div className="archive-controls performer-label-filters" aria-label="Filter performers by label"><span>FILTER BY LABEL</span><div className="archive-filters">{labelOptions.map(([value, label]) => <a key={value || 'all'} className={!labelFilter && !value || labelFilter === value ? 'active' : ''} href={value ? `/v3/performers?label=${encodeURIComponent(value)}` : '/v3/performers'}>{label}</a>)}</div></div>}{error ? <p className="error">{error}</p> : kind === 'videos' ? <ArchiveGrid records={records}/> : <div className="people-index">{records.map((item, index) => <PerformerSheet key={item.id || item.slug} performer={item} index={index}/>)}</div>}</main></Shell>;
}

function FilmMetadataBlock({ record }) { const performers = record.performers?.map(item => item.name).filter(Boolean).join(' · '); return <div className="film-file"><div><span>FILE</span><strong>{record.slug || 'PUBLIC-ENTRY'}</strong></div><div><span>PERFORMERS</span><strong>{performers || 'FLESHLAB / UNLISTED'}</strong></div><div><span>RUN TIME</span><strong>{formatRuntime(record.duration_seconds || record.runtime_seconds) || 'UNKNOWN'}</strong></div><div><span>RELEASED</span><strong>{record.release_date || 'PUBLIC ARCHIVE'}</strong></div><div><span>LABEL</span><strong>{record.brand_name || 'FLESHLAB'}</strong></div></div>; }
