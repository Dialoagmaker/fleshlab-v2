...
function FleshFooter() {
  return <footer className="flesh-footer"><div className="footer-lockup"><a href="/v3/home" className="flesh-logo"><strong>FLESH<span>LAB</span><sup>®</sup></strong><small>AMATEUR WINS.</small></a></div><div><span>ARCHIVE</span><a href="/v3/videos">All tapes</a><a href="/v3/performers">People</a><a href="/v3/collections">Series</a><a href="/v3/brands">Labels</a><a href="https://earn.fleshlab.online">Earn</a></div><div><span>ABOUT</span><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="/compliance">Contact</a></div><div><span>ACCOUNT</span><a href="https://performer.fleshlab.online">Log in ↗</a><a href="https://earn.fleshlab.online">Become a creator ↗</a></div><small className="footer-credit">© {new Date().getFullYear()} FLESHLAB. ALL RIGHTS RESERVED.</small></footer>;
}
...
function TapeCard({ video, index = 0, featured = false }) {
  return <a className={`tape-card${featured ? ' tape-card-featured' : ''}`} href={videoHref(video)} aria-label={`Open ${labelFor(video)}`}><Image record={video} alt={labelFor(video)} eager={featured}/><div className="tape-noise"/><div className="tape-top"><span>{tapeCode(index)}</span><span>{featured ? 'NEW STORY' : 'RAW CUT'}</span></div><div className="tape-bottom"><h3>{esc(video.title)}</h3><FilmMetadata video={video} index={index}/><span className="tape-open">VIEW ENTRY +</span></div></a>;
}

function SectionLabel({ number, title, detail, action, href }) { return <div className="archive-section-head"><div><span className="section-number">{number || '01'} /</span><h2>{title}</h2>{detail && <span className="section-detail">{detail}</span>}</div>{href && <a href={href}>{action || 'View all'} →</a>}</div>; }
...
function EarnPoster({ record }) { return <section className="earn-poster public-main"><div className="earn-image"><Image record={record || {}} alt=""/><span className="earn-image-note">YOUR STORY<br/>BELONGS<br/>HERE.</span><RecBadge/></div><div className="earn-copy"><span className="poster-kicker">OPEN CALL</span><h2>YOU DON'T NEED A STUDIO.</h2><p>REAL GUYS. REAL PHONES. REAL OPPORTUNITY.<br/>CREATE ON YOUR TERMS AND GET PAID.</p><strong>EARN UP TO $1 PER APPROVED MINUTE.</strong><a className="raw-button raw-button-red" href="https://earn.fleshlab.online">BECOME A CREATOR →</a></div><div className="earn-steps"><span>▧ &nbsp; SHOOT ON YOUR PHONE</span><span>↥ &nbsp; UPLOAD YOUR CONTENT</span><span>$ &nbsp; EARN UP TO $1 / MINUTE</span><span>♧ &nbsp; KEEP YOUR CREATIVE CONTROL</span></div><div className="earn-stamp">FLESHLAB®<br/><small>INDEPENDENT<br/>AMATEUR MEDIA<br/>ALWAYS REAL</small></div></section>; }

function BrandHome() {
  const [data, setData] = useState(null); const [error, setError] = useState('');
  useEffect(() => { get('/homepage').then(setData).catch(e => setError(e.message)); }, []);
  const hero = data?.hero; const featured = data?.featured_videos || []; const releases = data?.new_releases || []; const performers = data?.featured_performers || [];
