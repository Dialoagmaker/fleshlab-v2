...
function FleshFooter() {
  return <footer className="flesh-footer">
    ...
    <div><span>ABOUT</span><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="/compliance">Contact</a></div>
    <div><span>SUPPORT / ACCOUNT</span><a href="https://performer.fleshlab.online">Log in ↗</a><a href="https://earn.fleshlab.online">Become a creator ↗</a></div>
...
function PerformerSheet({ performer, index = 0 }) { const place = performer.public_location || performer.location || performer.country; const detail = place || (performer.video_count != null ? `${performer.video_count} appearances` : 'Public file'); return <a className="performer-sheet" href={performerHref(performer)}><Image record={performer} alt={`Portrait of ${performer.display_name}`}/><div className="sheet-mark">#{String(index + 1).padStart(3, '0')}</div><div className="sheet-caption"><strong>{esc(performer.display_name)}</strong><span>{esc(detail)} <b>+</b></span></div></a>; }
